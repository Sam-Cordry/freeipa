...
from ipalib.parameters import Certificate, Principal
...
from ipapython.ipautil import TMP_PWD_ENTROPY_BITS, ipa_generate_password
        ...
        if not isinstance(nsaccountlock, (bool, Bool)):
            ...
            if nsaccountlock.lower() not in ("true", "false"):
                raise errors.ValidationError(
                    name="nsaccountlock", error=_("must be TRUE or FALSE")
                )
...
def normalize_user_principal(value):
    ...
    lowercase_components = (principal.username.lower(),) + principal.components[1:]

    return unicode(kerberos.Principal(lowercase_components, realm=principal.realm))


def fix_addressbook_permission_bindrule(
    name, template, is_new, anonymous_read_aci, **other_options
):
...
def validate_passkey(ugettext, key):
    ...
    """
    pattern = re.compile(r"^passkey:(?P<id>[^,]*),(?P<pkey>[^,]*),?(?P<userid>.*)$")
    ...
    try:
        ...
        load_pem_public_key(data=pem.encode("utf-8"), backend=default_backend())
    except ValueError:
...
class baseuser(LDAPObject):
    ...
    attribute_members = {
        "manager": ["user"],
        "memberof": ["group", "netgroup", "role", "hbacrule", "sudorule", "subid"],
    ...
    takes_params = (
        ...
        Str("telephonenumber*", cli_name="phone", label=_("Telephone Number")),
        Str("mobile*", label=_("Mobile Telephone Number")),
        Str("pager*", label=_("Pager Number")),
        Str(
        ...
        try:
            ...
            for i, mgr in enumerate(manager):
                ...
                entry_attrs = self.backend.find_entry_by_attr(
                    self.primary_key.name, mgr, self.object_class, [""], container_dn
                )
                manager[i] = entry_attrs.dn
        except errors.NotFound:
            raise errors.NotFound(
                reason=_("manager %(manager)s not found") % dict(manager=mgr)
            )
    ...
    def convert_usercertificate_pre(self, entry_attrs):
        if "usercertificate" in entry_attrs:
            entry_attrs["usercertificate;binary"] = entry_attrs.pop("usercertificate")

    def convert_usercertificate_post(self, entry_attrs, **options):
        if "usercertificate;binary" in entry_attrs:
            entry_attrs["usercertificate"] = entry_attrs.pop("usercertificate;binary")
...
class baseuser_add(LDAPCreate):
    ...
    def pre_common_callback(self, ldap, dn, entry_attrs, attrs_list, *keys, **options):
        assert isinstance(dn, DN)
        set_krbcanonicalname(entry_attrs)
        self.obj.convert_usercertificate_pre(entry_attrs)
        if entry_attrs.get("ipatokenradiususername", None):
            add_missing_object_class(
                ldap, "ipatokenradiusproxyuser", dn, entry_attrs, update=False
            )
        if entry_attrs.get("ipauserauthtype", None):
            add_missing_object_class(
                ldap, "ipauserauthtypeclass", dn, entry_attrs, update=False
            )
        if entry_attrs.get("ipaidpconfiglink", None) or entry_attrs.get(
            "ipaidpsub", None
        ):
            add_missing_object_class(ldap, "ipaidpuser", dn, entry_attrs, update=False)
...
class baseuser_mod(LDAPUpdate):
    ...
    def check_name(self, entry_attrs):
        if "uid" in entry_attrs:
            ...
            if self.NAME_PATTERN.match(entry_attrs.single_value["uid"]) is None:
                raise errors.ValidationError(
                    name="uid", error=constants.ERRMSG_GROUPUSER_NAME.format("user")
                )
    ...
    def preserve_krbprincipalname_pre(self, ldap, entry_attrs, *keys, **options):
        ...
        try:
            old_entry = ldap.get_entry(
                entry_attrs.dn, attrs_list=("krbprincipalname", "krbcanonicalname")
            )
        ...
        self.context.krbprincipalname = old_entry.get("krbprincipalname", [])
    ...
    def preserve_krbprincipalname_post(self, ldap, entry_attrs, **options):
        ...
        principals_to_add = tuple(
            p for p in self.context.krbprincipalname if p != canonical_name
        )

        if principals_to_add:
            result = self.api.Command.user_add_principal(obj_pkey, principals_to_add)[
                "result"
            ]
    ...
    def check_objectclass(self, ldap, dn, entry_attrs):
        ...
        if special_attrs.intersection(entry_attrs):
            ...
            if (
                "ipauserauthtype" in entry_attrs
                and "ipauserauthtypeclass" not in obj_classes
            ):
...
class baseuser_find(LDAPSearch):
    ...
    def args_options_2_entry(self, *args, **options):
        ...
        return super(baseuser_find, self).args_options_2_entry(*args, **options)
    ...
    def common_enhance_options(self, newoptions, **options):
        ...
        if manager is not None:
            newoptions["manager"] = self.obj.normalize_manager(
...
class ModCertMapData(LDAPModAttribute):
    ...
    @classmethod
    def _convert_options_to_certmap(
        cls, entry_attrs, issuer=None, subject=None, certificates=()
    ):
        """
        ...
        for cert in certificates:
            ...
            if not subject:
                raise errors.ValidationError(
                    name="certificate", error=_("cannot have an empty subject")
                )
            data.append(cls._build_mapdata(subject, issuer))
    ...
    def pre_callback(self, ldap, dn, entry_attrs, attrs_list, *keys, **options):
        ...
        if subject or issuer:
            if certificates:
                raise errors.MutuallyExclusiveError(
                    reason=_("cannot specify both subject/issuer " "and certificate")
                )
            if certmapdatas:
                raise errors.MutuallyExclusiveError(
                    reason=_("cannot specify both subject/issuer " "and ipacertmapdata")
                )
        ...
        self._convert_options_to_certmap(
            entry_attrs, issuer=issuer, subject=subject, certificates=certificates
        )
...
class baseuser_remove_certmapdata(ModCertMapData, LDAPRemoveAttribute):
