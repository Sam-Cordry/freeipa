...
from ipapython.dnsutil import DNSName, DNSResolver, resolve, resolve_ip_addresses
from ipapython.kerberos import Principal
from ipapython.pgp import PGPPublicKey
...
def normalize_pgppubkey(value):
    return re.fullmatch(
        r"-----BEGIN PGP PUBLIC KEY BLOCK-----\s*([\w\+\/=][\w\+\/=\s]*[\w\+\/=])\s*-----END PGP PUBLIC KEY BLOCK-----",
        value,
    ).group(1)


def validate_pgppubkey(ugettext, value):
    try:
        PGPPublicKey(value)
    except (ValueError, UnicodeDecodeError):
        return _("invalid PGP public key")
    else:
        return None
...
def print_replication_status(entry, verbose):
    ...
    if verbose:
        ...
        updatestatus = entry.single_value.get("nsds5replicalastupdatestatus")
...
def check_principal_realm_in_trust_namespace(
    api_instance, *suffixes, attr_name="krbprincipalname"
):
    """
    ...
    """
    trust_suffix_namespace = _collect_trust_namespaces(api_instance, add_local=False)
...
def check_principal_realm_supported(
    api_instance, *suffixes, attr_name="krbprincipalname"
):
    """
    Check that principal name's suffix does not overlap with UPNs and realm
    ...
    """
    trust_suffix_namespace = _collect_trust_namespaces(api_instance, add_local=True)
    ...
    for p in suffixes[-1]:
        ...
        conditions = [
            (realm.lower() not in trust_suffix_namespace),
        ...
        if any(conditions):
            raise errors.ValidationError(
                name=attr_name,
                error=_(
                    "realm or UPN suffix outside of supported realm "
